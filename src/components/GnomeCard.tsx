  ...
  const renderSingleItem = (
    ...
    return (
      <div className="d-flex flex-column mb-2">
        <div
          className={getAlignClassName() + "font-weight-bold text-left"}
          style={{ fontSize: "12px" }}
        >
          {label}
        </div>
  ...
  return (
    <Card raised className="gnome-card-main">
      <Card.Content className="m-2 d-flex flex-column justify-content-between">
        <Card.Header className="gnome-card-header">{name}</Card.Header>
        <Card.Meta className="text-left">
          <div
            className="text-center gnome-card-hair"
            style={{ backgroundColor: hair_color.toLowerCase() }}
          >
              ...
              <div className="gnome-card-friends">
                {showFriends &&
                  friends.map((friend) => (
                    <u
                      onClick={() => {
                        props.setGnomeFriendsView("" + friend);
                      }}
                    >
                      {friend}
                    </u>
