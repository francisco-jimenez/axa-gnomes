...
export const filterByName = (gnomes: Array<Gnome>, searchString: string) => {
  if (searchString) {
    let matches = collection.filter(gnomes, function (gnome: Gnome) {
      return (
        gnome.name.toLowerCase().indexOf(searchString.toLowerCase()) !== -1
      );
    });
    return collection.sortBy(matches, [
      function (gnome: Gnome) {
        return gnome.name.toLowerCase().indexOf(searchString.toLowerCase());
      },
    ]);
  } else {
    return gnomes;
...
export const filterByProfession = (
  gnomes: Array<Gnome>,
  profession: string
) => {
  if (profession) {
    return collection.filter(gnomes, function (gnome: Gnome) {
      return gnome.professions.map((prof) => prof.trim()).includes(profession);
    });
  } else {
    return gnomes;
  }
...
export const filterByHairColor = (gnomes: Array<Gnome>, hairColor: String) => {
  if (hairColor) {
    return collection.filter(gnomes, function (gnome: Gnome) {
      return gnome.hair_color === hairColor;
    });
  } else {
    return gnomes;
  }
...
export const filterByAge = (
  ...
  if (age) {
    return collection.filter(gnomes, function (gnome: Gnome) {
      return gnome.age > age.min && gnome.age < age.max;
    });
  } else {
    return gnomes;
  }
};
export const filterByHeight = (
  gnomes: Array<Gnome>,
  height: { min: number; max: number }
) => {
  if (height) {
    return collection.filter(gnomes, function (gnome: Gnome) {
      return gnome.height > height.min && gnome.height < height.max;
    });
  } else {
    return gnomes;
  }
...
export const filterByWeight = (
  ...
  if (weight) {
    return collection.filter(gnomes, function (gnome: Gnome) {
      return gnome.weight > weight.min && gnome.weight < weight.max;
    });
  } else {
    return gnomes;
  }
